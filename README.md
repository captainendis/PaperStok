...
> **TEYİT:** Varsayılan ambar stok sorgusu (`PaperStok.Core/Logo/LogoQueryTemplates.cs`) üç kademede doğrulandı: [logoisortagim.com.tr'nin tablo rehberi](https://logoisortagim.com.tr/blog-logo-veritabani-tablolari.html), [ugurozpinar/Logo](https://github.com/ugurozpinar/Logo) deposundaki alan bazlı şema dökümü ve **gerçek bir müşteri veritabanına karşı çalıştırılan salt okunur bir keşif sorgusu**. Sonuncusu en güçlü kanıt — ve iki yazılı kaynağın da yanıldığını ortaya çıkardı: ambar tablosu `L_CAPIDEF` değil, **`L_CAPIWHOUSE`**. Alan (kolon) düzeyinde doğrulanan kısımlar: `STINVTOT` (`STOCKREF`, `INVENNO` — `-1` "tüm ambarlar" anlamına gelir ve sorguda dışlanır —, `ONHAND`, `RESERVED`; `ORDERED` diye bir kolon yok, en yakın karşılık `ACTPORDER`/"Verilen Siparişler"dir), `ITEMS` (`CODE`, `NAME`, `CARDTYPE`, `ACTIVE`, `UNITSETREF`), birim çözümlemesi (`UNITSETF` birim setinin kendisidir, gerçek birim adı `UNITSETL.CODE`'dan `MAINUNIT` bayrağıyla gelir). Tablo adlandırma kalıbı (`LG_<firma>_<dönem>_TABLOADI`) doğrulandı. `L_CAPIWHOUSE`'un kendi kolonları (`NR`/`NAME`/`FIRMNR`) henüz kolon düzeyinde doğrulanmadı — hâlâ aynı `L_CAPI*` ailesindeki kardeş tablolardan (`L_CAPIFIRM`, `L_CAPIUNIT` vb.) esinlenen bir varsayım. Sonuç satırları esnek tür dönüşümüyle okunur (`Convert.ToXxx`), bu yüzden Logo'nun `decimal` yerine `float` kullandığı alanlar sorun çıkarmaz. Müşteriye özel Logo kurulumlarında alan adları yine de farklılaşabilir; Bağlantı Ayarları ekranındaki "Gelişmiş: Özel Ambar Sorgusu" alanından ortamınıza göre uyarlayın.
