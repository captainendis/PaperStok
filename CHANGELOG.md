...
- Ana ekrandaki ve Ambar Raporu'ndaki tablolara sağ tıklayınca sütunları gösterme/gizleme menüsü eklendi. Ayrıca her iki ekrana da "Sütunları Sığdır" butonu eklendi: basıldığında sütun genişlikleri o anki içeriğe göre bir kerelik ayarlanır, sonrasında normal şekilde elle yeniden boyutlandırılabilir (buton tekrar basılana kadar kendiliğinden değişmez).
- Bağlantı Ayarları'na "Linked Server (ops.)" alanları eklendi: bir profil, bağlandığı sunucudaki Logo tablolarını doğrudan değil, o sunucuda tanımlı adlandırılmış bir SQL Server linked server üzerinden dört parçalı adla (`LinkedServer.UzakVeritabanı.dbo.<tablo>`) okuyabiliyor artık — gerçek Logo veritabanı ayrı bir sunucuda olup mevcut sunucuya yalnızca linked server ile bağlıyken kullanışlı. Boş bırakılırsa (varsayılan) hâlâ normal yerel bağlantı kullanılır; birden fazla profil işaretlenip "Stokları Çek"e basıldığında bu tür bir profil de diğerleriyle aynı tabloda birleşir. Linked Server doldurulduysa sorgular zaten tam nitelenmiş adres kullandığından "Veritabanı Adı" alanı zorunlu olmaktan çıkarıldı.
### Değiştirildi
